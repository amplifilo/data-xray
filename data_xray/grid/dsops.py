...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
'''new class style and switching to proplot'''

class grid_viz(object):
    def __init__(self):
        pass
    
    @classmethod
    def get_bias_index(cls, ds,bias=[0]):
        """
    ...
    @classmethod
    def find_index(cls, ds,chan,val):
        """
    ...
    @classmethod
    def chan_histogram(cls, ds, xy=['bias', 'cf'], xymod=[lambda x: x, lambda x: x], ax=0, label=['', '', ''], px=None):
        ...
        if ax == 0:
            ...
            hist = ax.hist2d(a2.x, a2.y, bins=(100, 100), cmap='jet',robust=True)
            
            ax.set_xlabel(label[0])
            ax.set_ylabel(label[1])
            ax.set_title(label[2])
            ax.colorbar(hist[3], loc='r')
    
    @classmethod
    def chan_method(cls, ds, chan, mod=lambda x: x):
        """
    ...
    @classmethod
    def chan_pca_denoise(cls, ds, xvec='bias', chan='cf',keep_components=4):
        """
        ...
        cf_dn = cls.pca_denoise(pca_dat=cf, keep_components=keep_components)
    ...
    @classmethod
    def chan_pca_kmeans(cls, ds, xvec='bias', chan='cf', mod=lambda y: y, comps=6,nclust=4, fig=None):
        """
    ...
    @classmethod
    def chan_mod(cls, ds, chan, mod=lambda x: x):
        """
        apply any well-behaved lambda function to a specific channel within a dataset
        :param ds: xarray dataset
        :param chan: channel to apply modification too
    ...
    @classmethod
    def subtract_zoffset(cls, ds=None, chan=['zr', 'zf']):
        """
        ...
        try:
            cls.chan_mod(ds, chan=chan, mod=lambda x: (x - x[:, :, 0]))
            print('z_subtract..all done!')
        except:
            print('..no changes made. Check params')

    @classmethod
    def substract_current_offset(cls, ds=None, chan=['cf', 'cr'], vrange=None):
        """
    ...
    @classmethod
    def chan_derivative(cls, ds=None,chan='cf',xvec='bias'):
        """
